...
//				TODO notify ?
	...
	public long checkError(final long argsP) {
		this.dbg.beginSafeMode();
		try {
			...
			final int flags= 0;
			synchronized (this.tracepointMap) {
				final List<TracepointState> list= this.tracepointMap.get("exception");
				if (list != null) {
	...
	void handleSuspended(final long srcrefP) {
		...
		if (state != null) {
			...
			if (state.getType() == Tracepoint.TYPE_EB
					|| (srcrefP != 0 && Arrays.equals(this.hitBreakpointSrcref, this.rEngine.rniGetIntArray(srcrefP)) )) { // compare srcfile?
				this.dbg.sendNotification(new TracepointEvent(TracepointEvent.KIND_ABOUT_TO_HIT,
						...
						(state.getType() == Tracepoint.TYPE_EB) ? state.getElementId() : state.getElementLabel(),
